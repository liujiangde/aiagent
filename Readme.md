...
### `npm run dev -w web` 命令

该命令用于在特定工作区中启动开发模式：

- `npm run dev`: 执行开发模式脚本，实际调用 `turbo run dev --filter=!aiagent-monorepo`
- `-w web`: 指定只在名为 [web](file:///Users/liujiang/Desktop/aiagent/node_modules/web) 的工作区中运行该命令

**整体效果**：仅在 [web](file:///Users/liujiang/Desktop/aiagent/node_modules/web) 工作区中启动开发服务器，而非所有子工作区

**使用场景**：
- 在 monorepo 结构中精确控制命令执行范围
- 单独为某个应用或包启动开发环境
- 避免同时启动所有工作区，提高开发效率

**注意**：[web](file:///Users/liujiang/Desktop/aiagent/node_modules/web) 工作区通常位于 `apps/web` 或 `packages/web` 目录下
